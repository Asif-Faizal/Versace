package category

import (
	"encoding/json"
	"fmt"
	"log"
	"mime/multipart"
	"net/http"

	"github.com/Asif-Faizal/Versace/types/category"
	"github.com/Asif-Faizal/Versace/utils"
	"github.com/xuri/excelize/v2"
)

func (h *Handler) handleBulkCreateCategory(w http.ResponseWriter, r *http.Request) {
	err := r.ParseMultipartForm(10 << 20) // 10 MB
	if err != nil {
		utils.WriteError(w, http.StatusBadRequest, "failed to parse multipart form", err.Error())
		return
	}

	file, _, err := r.FormFile("sheet")
	if err != nil {
		utils.WriteError(w, http.StatusBadRequest, "excel sheet is required", err.Error())
		return
	}
	defer file.Close()

	excelFile, err := excelize.OpenReader(file)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, "failed to read excel file", err.Error())
		return
	}

	rows, err := excelFile.GetRows("Sheet1")
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, "failed to get rows from excel sheet", err.Error())
		return
	}

	formFiles := r.MultipartForm.File["images"]
	if len(formFiles) == 0 {
		utils.WriteError(w, http.StatusBadRequest, "at least one image is required", "")
		return
	}

	filesMap := make(map[string]*multipart.FileHeader)
	for _, fh := range formFiles {
		filesMap[fh.Filename] = fh
	}

	var categoriesToCreate []*category.Category

	for i, row := range rows {
		if i == 0 { // Skip header row
			continue
		}
		if len(row) < 3 {
			log.Printf("skipping row %d, not enough columns", i+1)
			continue
		}
		categoryName := row[0]
		categoryDesc := row[1]
		imageFileName := row[2]

		imageFileHeader, ok := filesMap[imageFileName]
		if !ok {
			log.Printf("image %s for category %s not found in uploaded files, skipping", imageFileName, categoryName)
			continue
		}

		imageURL, err := h.supabaseService.UploadFile(imageFileHeader, "images")
		if err != nil {
			log.Printf("failed to upload image %s for category %s: %v, skipping", imageFileName, categoryName, err)
			continue
		}

		categoriesToCreate = append(categoriesToCreate, &category.Category{
			Name:        categoryName,
			Description: categoryDesc,
			ImageURL:    imageURL,
		})
	}

	if len(categoriesToCreate) == 0 {
		utils.WriteError(w, http.StatusBadRequest, "no categories to create", "please check excel file and images")
		return
	}

	err = h.store.BulkCreateCategory(categoriesToCreate)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, "failed to create categories", err.Error())
		return
	}

	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusCreated)
	json.NewEncoder(w).Encode(map[string]string{"message": fmt.Sprintf("%d categories created successfully", len(categoriesToCreate))})
}
